...
class TransactionModel{
    ...
    public function __construct()
    {
        ...
        $host          = $config['bayarcash_db_host'];
        $dbname        = $config['bayarcash_db_dbname'];
        $username      = $config['bayarcash_db_username'];
        $password      = $config['bayarcash_db_password'];
        $charset       = "utf8mb4";
    ...
    public function getNewTransactions()
    {
       return $this->pdo
                   ->query('
                    ...
                    WHERE transaction_status=' . get_payment_status_name(0) .
                    'AND
    ...
    public function getNewTransactionsOrderRefNo()
    {
       return $this->pdo
                   ->query('
                    SELECT order_ref_no FROM transactions
                    WHERE transaction_status=' . get_payment_status_name(0) .
                    'AND
                    DATE(created_at) = curdate()
                   ')
    ...
    {
       return $this->pdo
                    ->query('SELECT * FROM transactions WHERE transaction_status='.get_payment_status_name(3))
                    ->fetchAll();
    }
    ...
    {
       return $this->pdo
                    ->query('SELECT * FROM transactions WHERE transaction_status'.get_payment_status_name(2))
                    ->fetchAll();
    }
    ...
    public function insert($transaction)
    {
        return $this->pdo->prepare(
            ...
            'buyer_bank_name' => get_payment_status_name(0) ?? null, 
            'transaction_status' =>  $transaction['transaction_status'] ?? 0, 
    ...
    public function update($transaction)
    {
        ...
        ')->execute([
            ...
            'transaction_status' => get_payment_status_name($transaction['transaction_status']) ?? null, 
            'transaction_status_description' => $transaction['transaction_status_description'] ?? null,
            'transaction_datetime' => $transaction['transaction_datetime'] ?? null, 
            'transaction_gateway_id' => $transaction['transaction_gateway_id'] ?? null,
            'order_ref_no' => $transaction['order_ref_no'] ?? null 
